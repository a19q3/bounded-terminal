...
AI coding agent 把这个循环放大了。它们会更频繁地跑测试、搜索代码、读取文件、调用 formatter、执行生成脚本、检查 diff。单个动作看起来都很合理，但合在一起就会出现一种新的浪费：终端没有边界，agent 的上下文窗口却有边界。一次 noisy `cargo test`、一次全仓 `rg`、一次误读整文件，都会把大量低价值文本塞进会话里。模型随后还要在这些噪声里推理，仿佛让一位博士后在传真机纸堆里找牙签。

这不是“AI 不够聪明”的问题，也不完全是 prompt engineering 的问题。很多时候，问题在于我们仍然把终端工具当成给人类眼睛设计的交互界面，而 agent 需要的是更明确的操作 receipt：命令真实退出码是什么，可见输出有多少，完整日志在哪里，当前行属于哪个语法单元，命令到底改了哪些文件，管道里流过的数据大概长什么样。

换句话说，现代 agent 终端工作已经不只是：
...
```text
cargo test             -> 输出可能巨大
cat src/main.rs        -> 上下文可能过宽
cargo fmt              -> 文件副作用可能不清楚
producer | jq | sink   -> 中间数据可能被 dump
```

`cap`、`span`、`fx`、`tap` 正好对应这四个陷阱。它们不是一套宏大的平台，而是一组很小的刹车片。刹车片听起来也不浪漫，但没有它们，跑车只是一种昂贵的事故预告。

所以这套工具的立场很明确：不替代 shell，不替代 Git，不替代 IDE，也不替代 AST 工具。它们只是在 agent 经常踩空的地方加四个小护栏。`cap` 管输出边界，`span` 管代码上下文边界，`fx` 管文件副作用可见性，`tap` 管管道数据流观察。每个工具都应该能被人类单独使用，也应该能被 agent 可靠组合；如果一个工具只能在营销图里成立，那它最好留在营销图里。
